...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   check_and_run_builtins.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: htopa <htopa@student.hive.fi>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/20 22:38:06 by htopa             #+#    #+#             */
/*   Updated: 2025/04/22 20:44:24 by htopa            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	free_before_exit(t_shell *shell, t_cmd_parts **cmd_parts,
	t_args *arg_struct)
{
...
static int	exit_w_more_than_two_args(t_shell *shell,
	t_cmd_parts **cmd_parts, t_args *arg_struct)
{
	...
	else
	{
		...
		free_before_exit(shell, cmd_parts, arg_struct);
		exit(2);
	}
}

static int	run_exit(t_shell *shell, t_cmd_parts **cmd_parts,
	t_args *arg_struct)
{
	int	exit_code;
	int	len;
	...
	{
		exit_code = exit_w_more_than_two_args(shell, cmd_parts, arg_struct);
		return (exit_code);
	}
}
	...
	t_args *arg_struct)
{
	if (ft_strncmp((*cmd_parts)->cmd_array[0], "unset\0", 6) == 0)
		return (run_unset(cmd_parts, &(arg_struct->envp)));
	else if (ft_strncmp((*cmd_parts)->cmd_array[0], "export\0", 7) == 0)
		return (run_export(cmd_parts, &(arg_struct->envp)));
		...
		return (run_env(cmd_parts, &(arg_struct->envp)));
	else if (ft_strncmp((*cmd_parts)->cmd_array[0], "cd\0", 3) == 0)
		return (run_cd(cmd_parts, &(arg_struct->envp)));
	else if (ft_strncmp((*cmd_parts)->cmd_array[0], "exit\0", 5) == 0)
		return (run_exit(shell, cmd_parts, arg_struct));
